...
def log_prob(target, mask, logits):
    """
...
def weighted_log_prob(target, mask, topic_prob, logits):
    """
    logits          : shape [batch, seq_len, num_tokens]
    target : shape [batch, seq_len]
    prob      : shape [batch]
    ...
    """
    probs = torch.softmax(logits, dim=2)
    target_p = torch.gather(probs, 2, target.unsqueeze(2)).squeeze(2)
    seq_len_mask = mask.sum(dim=1)
    avg_log_p = ((torch.log(target_p) * mask)/ seq_len_mask.unsqueeze(1)).sum(dim=1) 
    return (avg_log_p * topic_prob).mean()
